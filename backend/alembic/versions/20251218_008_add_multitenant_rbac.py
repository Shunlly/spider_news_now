...
def upgrade() -> None:
    ...
    # verification_expires - 验证 Token 过期时间
    ...
    op.add_column('users', sa.Column('login_attempts', sa.Integer(), nullable=False, server_default='0', comment='连续登录失败次数'))
    # locked_until - 账号锁定截止时间
    op.add_column('users', sa.Column('locked_until', sa.DateTime(), nullable=True, comment='账号锁定截止时间'))
