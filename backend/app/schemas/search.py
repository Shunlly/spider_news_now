...
class SearchRequest(BaseModel):
    ...
    source_key: str | None = Field(None, description="按来源过滤")
    category: str | None = Field(None, description="按分类过滤")
    start_date: datetime | None = Field(None, description="时间范围起始")
    end_date: datetime | None = Field(None, description="时间范围结束")
...
class SearchHitResponse(BaseModel):
    ...
    # 高亮字段（可选）
    title_highlighted: str | None = Field(None, description="高亮后的标题")
    content_highlighted: str | None = Field(None, description="高亮后的内容片段")
...
class SearchResponse(BaseModel):
    ...
    # 分面统计（可选）
    facets: FacetDistributionResponse | None = Field(
